# datasets/

Required dataset categories:

* Patient message / healthcare conversation dataset
* Appointment or scheduling dataset
* Hospital satisfaction or patient experience dataset
* EHR-style patient records dataset
* Specialty or symptom classification dataset


Only this processed subset is packaged into Azure.

**The main reason was that I wanted to make this demo cheaper in terms of token usage for the later steps :)**
